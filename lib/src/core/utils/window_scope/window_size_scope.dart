...
class WindowSize extends Size implements Comparable<WindowSize> {
  ...
  /// Returns "white space" padding based on the current breakpoint.
  /// This padding is used to create a "white space" between the edge of the screen and the content.
  EdgeInsets padding() {
    return map(
      compact: () => EdgeInsets.symmetric(horizontal: width * 0.1),
      medium: () => EdgeInsets.symmetric(horizontal: width * 0.1),
      expanded: () => EdgeInsets.symmetric(horizontal: width * 0.1),
      large: () => EdgeInsets.symmetric(horizontal: width * 0.2),
      extraLarge: () => EdgeInsets.symmetric(horizontal: width * 0.3),
    );
  }

  /// Returns `true` if the viewport width is within the range of the compact breakpoint.
  bool get isCompact => maybeMap(compact: () => true, orElse: () => false);
  ...
  bool get isMedium => maybeMap(medium: () => true, orElse: () => false);
  ...
  bool get isExpanded => maybeMap(expanded: () => true, orElse: () => false);
